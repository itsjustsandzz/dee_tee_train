...
def VectMax(key, *data, debug=False):
    ...
    res = util.tree_reduce(reducer, zip(key, *data))[2:]
    if debug:
...
def ModifiedGini(g, y, debug=False):
    ...
    if debug:
        print_ln('gini %s %s', util.reveal(res_num), util.reveal(res_den))
...
class TreeTrainer:
    ...
    def AttributeWiseTestSelection(self, g, x, y, time=False, debug=False):
        ...
        if time:
            start_timer(2)
        s_num, s_den = ModifiedGini(g, y, debug=debug or self.debug > 2)
        if time:
            stop_timer(2)
        if debug or self.debug > 1:
            print_ln('gini %s %s', s_num.reveal(), s_den.reveal())
        xx = x
        ...
        s_num = p[:].if_else(MIN_VALUE, s_num)
        s_den = p[:].if_else(1, s_den)
        t = p[:].if_else(MIN_VALUE, t[:])
        if debug:
            print_ln('attribute s_num=%s s_den=%s t=%s', util.reveal(s_num), util.reveal(s_den), util.reveal(t))
        if time:
            start_timer(3)
        s_num, s_den, t = GroupMax(gg, s_num, s_den, t)
        if time:
            stop_timer(3)
        if debug:
            print_ln('attribute s_num=%s s_den=%s t=%s', util.reveal(s_num), util.reveal(s_den), util.reveal(t))
        return t, s_num, s_den
    ...
    def GlobalTestSelection(self, x, y, g):
        ...
        s_num = sint.Matrix(m, n)
        s_den = sint.Matrix(m, n)
        @for_range_multithread(self.n_threads, 1, m)
        def _(j):
            ...
            if self.debug_threading:
                print_ln('global sort %s %s %s', j, util.reveal(u[j]),
                         util.reveal(v[j]))
            t[j][:], s_num[j][:], s_den[j][:] = self.AttributeWiseTestSelection(
        ...
        if self.time:
            start_timer(4)
        if self.debug > 1:
            print_ln('s_num=%s', s_num.reveal_nested())
            print_ln('s_den=%s', s_den.reveal_nested())
            print_ln('t=%s', t.reveal_nested())
        a[:], tt = VectMax((s_num[j][:] for j in range(m)), (s_den[j][:] for j in range(m)), range(m),
